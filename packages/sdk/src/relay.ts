...
export type Relay = {
  ...
  createFetchForChain: (chainId: number) => ReturnType<typeof createRelayFetch> | null;
  createFetch: (originalUrl?: string) => (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>;
};
...
export const createRelay = async (options: RelayOptions): Promise<Relay> => {
  ...
  const createFetchForChain = (chainId: number) => {
  ...
  const createFetch = (originalUrl?: string) => {
    ...
    return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      ...
      if (detectedChainId !== undefined && detectedChainId !== null && supportsChain(detectedChainId)) {
        ...
        const fn = createFetchForChain(detectedChainId);
        if (fn) {
